...
class DeleteProduct(DeleteView):
    ...
    success_url = reverse_lazy('products')          # reverse lazy is used to eliminate the big url path 


def searchView(request):
    query = request.GET.get('q1')               # GET is a type of request  .get is a method
    results = Product.objects.filter(name__icontains=query)  # filtering the product objects according to string matching query
